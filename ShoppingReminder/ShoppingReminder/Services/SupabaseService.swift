...
class SupabaseService: ObservableObject {
    ...
    private init() {
        ...
        Task {
            do {
                ...
                #if DEBUG
                SecureLog.debug("DEBUG: Initial session check failed")
                #endif
            }
    ...
    // 認証: アプリ専用の匿名アカウント開始
    func startAppAccount(displayName: String) async throws {
        let trimmedName = displayName.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmedName.isEmpty else { throw NSError(domain: "Auth", code: 400) }
    ...
    func getGroupByCode(code: String) async throws -> Group? {
        ...
        struct FindGroupRequest: Encodable {
            let code: String
        }

        struct FindGroupResponse: Decodable {
            let group: Group?
        }

        let response: FindGroupResponse = try await client.functions.invoke(
            "find-group-for-join",
            options: FunctionInvokeOptions(body: FindGroupRequest(code: trimmedCode))
        )
    ...
    func deleteItem(id: UUID, imageUrl: String? = nil) async throws {
        ...
        if let imageUrl, !imageUrl.isEmpty,
           let storagePath = extractStoragePath(from: imageUrl) {
            ...
            _ = try? await client.storage.from(itemImagesBucket).remove(paths: [storagePath])
        }
    ...
    func savePushToken(token: String, deviceId: String) async {
        guard let userId = currentUser?.id else {
            #if DEBUG
            SecureLog.debug("[PushToken] スキップ - ログイン未完了")
            #endif
            return
        }
        ...
        do {
            ...
            try await client
                ...
                .execute()
            #if DEBUG
            SecureLog.debug("[PushToken] 保存成功")
            #endif
        } catch {
            #if DEBUG
            SecureLog.debug("[PushToken] 保存失敗")
            #endif
        }
    }
    ...
    func deleteAccount() async throws {
        ...
        // 3. 自身のデータを完全に削除するEdge Functionを実行
        ...
        do {
            ...
            #if DEBUG
            SecureLog.debug("Account deleted; local sign-out cleanup failed")
            #endif
        }
    }
}
